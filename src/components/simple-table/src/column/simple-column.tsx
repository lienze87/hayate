...
const SimpleTableColumn = defineComponent({
  ...
  setup({ column }) {
    ...
    return () => {
      try {
        const { label, prop, width, type, extType, formatter, render, opts, customOpts } = config;
          ...
          case 'link':
            return (
              <el-table-column label={label} prop={prop} min-width={width} formatter={formatter} {...opts}>
                {{
                  default: (scope: ScopeProps) => typeRenders.link(scope, config),
                }}
              </el-table-column>
            );
          case 'btn':
            return (
              <el-table-column label={label} prop={prop} min-width={width} {...opts}>
                {{
                  default: (scope: ScopeProps) => typeRenders.btn(scope, config),
                }}
              </el-table-column>
            );
          case 'switch':
            return (
              <el-table-column label={label} prop={prop} min-width={width} {...opts}>
                {{
                  default: (scope: ScopeProps) => typeRenders.switch(scope, config),
                }}
              </el-table-column>
            );
          case 'custom':
