      ...
      <ul class="toc-items">
        ...
        >
          <a
            ...
            :href="link"
            :title="text"
            @click.stop.prevent="handleClick(link)"
          >
            {{ text }}
          </a>
          <ul v-if="children">
...
const scrollToElement = (ele: Element) => {
  const headerOffset =
    (document.querySelector(".fixed-header") as HTMLElement)?.offsetHeight || 0;
  const stickyOffset =
    (document.querySelector(".sticky") as HTMLElement)?.offsetHeight || 0;
  const y =
