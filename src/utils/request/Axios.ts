...
export class VAxios {
  ...
  request<T = any>(config: AxiosRequestConfigRetry, options?: RequestOptions): Promise<T> {
    ...
    if (requestOptions && requestOptions.throttle !== undefined && requestOptions.debounce !== undefined) {
      throw new Error('throttle and debounce cannot be set at the same time');
    }

    if (requestOptions && requestOptions.throttle && requestOptions.throttle.delay !== 0) {
      return new Promise((resolve) => {
        throttle(() => resolve(this.synthesisRequest(config, options)), requestOptions.throttle?.delay);
      });
    }

    if (requestOptions && requestOptions.debounce && requestOptions.debounce.delay !== 0) {
      return new Promise((resolve) => {
        ...
        .catch(async (e: Error | AxiosError) => {
          ...
          if (axios.isAxiosError(e)) {
            ...
            if (e.response?.data instanceof Blob) {
              const res = JSON.parse(await e.response?.data?.text());
              message = res ? res.message : '';
            }
            reject(message ? new Error(`错误信息: ${message}`) : e);
            return;
          }
          reject(e);
        });
